...
function typToMd(s){
  return s.replace(/^====\s+(.+)$/gm,'#### $1').replace(/^===\s+(.+)$/gm,'### $1').replace(/^==\s+(.+)$/gm,'## $1').replace(/^=\s+(.+)$/gm,'# $1').replace(/#image\("([^"]+)"\)/g,'![]($1)').replace(/#link\("([^"]+)"\)\[([^\]]+)\]/g,'[$2]($1)');
}
...
const files = (await fs.readdir(postsDir)).filter(f => (f.endsWith('.md') || f.endsWith('.typ')) && !f.startsWith('_'));
const posts = [];
for(const file of files){
  const raw = await fs.readFile(path.join(postsDir,file),'utf8');
  const [data, content] = meta(raw);
  const text = file.endsWith('.typ') ? typToMd(content) : content;
  const item = info(file, data, text);
  const out = path.join(blogDir, item.id);
  await fs.mkdir(out,{recursive:true});
  await fs.writeFile(path.join(out,'index.html'), shell({...item, body:md.render(text), raw:`/posts/${file}`}));
  posts.push(item);
}
